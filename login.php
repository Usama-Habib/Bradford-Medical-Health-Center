...
//        CHECK IF THE SAME EMAIL EXISTS IN THE DATABASE
        $select_user = $conn->prepare( "SELECT * FROM users WHERE BINARY email = ?");
        ...
        if ($select_user->execute()){
           ...
           $rowscount = $result->num_rows;
            if ($rowscount > 0){
//               Account exists, Now read password

                while ($data = $result->fetch_assoc()) {
                    if(password_verify($password,$data['password_hash'])){
                        $update_access = "UPDATE users SET last_access = NOW() WHERE email = '". $data['email'] ."'";
                        ...
                        $errors['login_err'] = 'Password doesn\'t match'  ;
                    }
                }
            }else{
//              No record found
...
<div class="wrapper">
    ...
    if ($errors['login_err'] ?? false){
        echo "<p style='max-width: 600px; width:600px; padding:5px; background-color: #008cba; border-radius:5px; text-align: center; font-style: italic font-size: 18px; color: white; '> ". $errors['login_err'] ." </p>";
    }
    ?>
  <div class="container1">
    <div class="col-left">
      <div class="login-text">
        ...
        <!-- <a class="btn" style="margin-bottom: 5px;"  href="doctor_register.php">Doctor Sign Up</a>
        <br> -->
